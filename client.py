...
# 在线用户列表
list_box = tkinter.Listbox(root1)
list_box.place(x=510, y=0, width=130, height=320)
...
def receive():
    ...
    while True:
        ...
        try:
            ...
            list_box.insert(tkinter.END, "当前在线用户")
            list_box.insert(tkinter.END, "------Group chat-------")
            for x in range(len(uses)):
                list_box.insert(tkinter.END, uses[x])
            ...
            chat_with = data[2]
            message = '\n' + message
            if chat_with == '------Group chat-------':   # 群聊
                if userName == user:
                    listbox.insert(tkinter.END, message)
                else:
                    listbox.insert(tkinter.END, message)
            elif userName == user or chat_with == user:  # 私聊
                if userName == user:
